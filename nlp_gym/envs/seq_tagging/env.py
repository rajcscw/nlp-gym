...
class SeqTagEnv(BaseEnv):
    ...
    def __init__(self, possible_labels: List[str], reward_function: RewardFunction = None,
        ...
        reward_function = EntityF1Score(
            dense=True, average="micro") if reward_function is None else reward_function
    ...
    def step(self, action: int) -> Tuple[Union[Observation, np.array], int, bool, dict]:
        ...
        if not done:
            ...
            self.current_sample.observation.current_action_history.append(
                action_str)
    ...
    def reset(self, sample: Sample = None) -> Union[Observation, np.array]:
        ...
        input_text_tokens = SeqTagEnv._tokenize(
            self.current_original_sample.input_text, sample.oracle_label)
        ...
        if self.observation_featurizer is not None:
            self.observation_featurizer.init_on_reset(
                self.current_original_sample.input_text)
    ...
    def render(self):
        ...
        history = [
            ""] if self.time_step == 0 else self.current_sample.observation.get_current_action_history()
        for token, predicted_label in zip(self.current_sample.text[:self.time_step+1], history):
            print(
                f"[italic red]Token[/italic red]:{token} -> [italic red]Label[/italic red]: {predicted_label}")
